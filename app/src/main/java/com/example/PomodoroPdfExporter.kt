...
object PomodoroPdfExporter {
    ...
    suspend fun exportToPdfAndShare(
        context: Context,
        summaries: List<PomodoroDailySummary>,
        sessionLogs: List<PomodoroSessionLog>
    ) {
        withContext(Dispatchers.Main) {
            Toast.makeText(context, "Generating focus summary PDF report...", Toast.LENGTH_SHORT).show()
        }

        val cacheFile = File(context.cacheDir, "Pomodoro_Focus_Report.pdf")

        val generationSuccess = withContext(Dispatchers.IO) {
            try {
        ...
        withContext(Dispatchers.Main) {
            if (generationSuccess && cacheFile.exists() && cacheFile.length() > 0) {
                try {
                    ...
                    e.printStackTrace()
                    Toast.makeText(context, "Failed to launch sharing: ${e.localizedMessage}", Toast.LENGTH_LONG).show()
                }
            } else {
                Toast.makeText(context, "Failed to generate PDF report file.", Toast.LENGTH_LONG).show()
            }
        }
    }
}
